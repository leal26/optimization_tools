...
if __name__ == "__main__":
    ...
    def g1(x):
        return True if x >= 1. else False  # Inequality Constraint
    # Lift higher than weight

    def g2(x, y):
        return True if abs(y - x) <= 500. else False  # Inequality Constraint
    g = [g1, g2]
    outputs_constrained = ['EigenValue', ['Weight', 'Lift']]
    outputs_plotted = ['Weight', 'Velocity']
    units = ['N', 'm/s']
    plot_generations(filename, g=g, output_constrained=outputs_constrained,
                     outputs_plotted=outputs_plotted,
                     n_generation=n_generation, units=units)
